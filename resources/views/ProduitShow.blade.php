...
<body>
    ...
    <form class="flex items-center max-w-sm mx-auto" method="post" action="/produitShow">
        @csrf
    ...
    <div class="mx-4 my-3">
        <a href="#" class="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
            <img class="object-cover w-40 h-40 rounded-t-lg md:h-auto md:w-48 md:rounded-none md:rounded-s-lg" src="uploads/Prd_images/{{$prd->image}}" alt="">
            <div class="flex flex-col justify-between p-4 leading-normal">
                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{$prd->nome}}</h5>
                <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">{{$prd->description}}</p>
            </div>
        </a>
    </div>
    @endforeach
